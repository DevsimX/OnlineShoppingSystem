export default function SparkleDecorations() {
  return (
    <>
      <div className="pointer-events-none absolute -top-2 -right-1.5 z-[2] hidden w-[75px] group-hover:block">
        <svg
          width="80"
          height="80"
          viewBox="0 0 160 160"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="sparkle-animation"
        >
          <g id="sparkles">
            <path
              id="Vector"
              d="M119.016 5.11628e-06C117.426 35.6198 115.493 37.6609 80 40.9844C115.62 42.5739 117.661 44.5067 120.984 80C122.574 44.3802 124.507 42.3391 160 39.0156C124.38 37.4261 122.339 35.4933 119.016 5.11628e-06Z"
              fill="white"
            />
            <path
              id="Vector_2"
              d="M39.0156 80C37.4261 115.62 35.4933 117.661 5.11628e-06 120.984C35.6198 122.574 37.6609 124.507 40.9844 160C42.5739 124.38 44.5067 122.339 80 119.016C44.3802 117.426 42.3391 115.493 39.0156 80Z"
              fill="white"
            />
          </g>
        </svg>
      </div>
      <div className="pointer-events-none absolute -bottom-2 -left-2 z-[2] hidden w-[75px] group-hover:block">
        <svg
          width="80"
          height="80"
          viewBox="0 0 160 160"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="sparkle-animation"
        >
          <g id="sparkles">
            <path
              id="Vector"
              d="M119.016 5.11628e-06C117.426 35.6198 115.493 37.6609 80 40.9844C115.62 42.5739 117.661 44.5067 120.984 80C122.574 44.3802 124.507 42.3391 160 39.0156C124.38 37.4261 122.339 35.4933 119.016 5.11628e-06Z"
              fill="white"
            />
            <path
              id="Vector_2"
              d="M39.0156 80C37.4261 115.62 35.4933 117.661 5.11628e-06 120.984C35.6198 122.574 37.6609 124.507 40.9844 160C42.5739 124.38 44.5067 122.339 80 119.016C44.3802 117.426 42.3391 115.493 39.0156 80Z"
              fill="white"
            />
          </g>
        </svg>
      </div>
    </>
  );
}
